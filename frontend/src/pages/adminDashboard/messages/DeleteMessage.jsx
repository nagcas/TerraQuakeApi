...
export default function DeleteMessage({ messages, setMessages }) {
  ...
  const handleDelete = async () => {
    try {
      ...
      const response = await api.delete(`/contact/${messages._id}`, {
        headers: {
      ...
      await Swal.fire({
        title: 'Deleted!',
        text: response.data.message || 'Message has been deleted successfully.',
        icon: 'success',
        confirmButtonText: 'Tabular View',
      });

      // Update the list
      setMessages((prev) =>
        prev.map((m) => (m._id === messages._id ? { ...m, deleted: true } : m)),
      );
            ...
            >
              ...
              <header className='p-5 bg-purple-400/20 border-b border-white/10 flex justify-between items-center'>
                <h2 className='text-white uppercase tracking-wider text-sm font-semibold'>
                  Delete Message id: {messages?._id}
                </h2>
              ...
              <div className='flex flex-col items-center gap-4 mt-20'>
                <p className='text-2xl text-center p-2 pt-5 text-gray-300'>
                  You are about to deactivate this message. This is a reversible
                  action and the message can be restored at any time.
                </p>
                <p className='text-xl text-center p-2 text-gray-300'>
                  Please confirm that you want to proceed with the temporary
                  deactivation of this message.
                </p>
              </div>
              ...
              <div className='flex justify-end p-6 gap-4 pt-4'>
                ...
                >
                  {loading ? <Spinner /> : 'Delete message'}
                </button>
