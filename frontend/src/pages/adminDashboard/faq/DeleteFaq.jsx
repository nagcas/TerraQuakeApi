...
export default function DeleteFaq({ faqs, setFaqs }) {
  ...
  const handleDelete = async () => {
    try {
      ...
      const response = await api.delete(`/faqs/${faqs._id}`, {
        headers: {
      ...
      await Swal.fire({
        title: 'Deleted!',
        text: response.data.message || 'Faq has been deleted successfully.',
        icon: 'success',
      ...
      // Update the list
      setFaqs((prev) =>
        prev.map((f) => (f._id === faqs._id ? { ...f, deleted: true } : f)),
      );
            ...
            >
              ...
              <div className='flex flex-col items-center gap-4 mt-20'>
                <p className='text-2xl text-center p-2 pt-5 text-gray-300'>
                  You are about to deactivate this faq. This is a reversible
                  action and the faq can be restored at any time.
                </p>
                <p className='text-xl text-center p-2 text-gray-300'>
                  Please confirm that you want to proceed with the temporary
                  deactivation of this faq.
                </p>
              </div>
