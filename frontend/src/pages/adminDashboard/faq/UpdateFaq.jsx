...
export default function UpdateFaq({ faqs, setFaqs }) {
  ...
  const updateFaqSchema = yup.object({
    question: yup.string().required(t('update_user.name_required')),
    answer: yup.string().required(t('email_required')),
  });
  ...
  const handleUpdateFaq = async (data) => {
    ...
    try {
      ...
      const response = await api.patch(`/faqs/${faqs._id}`, payload, {
        headers: { Authorization: `Bearer ${token}` },
      });

      const updatedFaq = response.data.payload || response.data;
          ...
          >
            ...
            >
              ...
              <div className='p-8 max-h-[70vh] overflow-y-auto'>
                <form
                  className='space-y-8'
                  onSubmit={handleSubmit(handleUpdateFaq)}
                >
                  {/* Input component */}
                  {[
                    {
