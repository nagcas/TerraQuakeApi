...
router.get(
  '/recent',
  getEarthquakesByRecent({ buildResponse, handleHttpError }),
);
...
router.get(
  '/last-week',
  getEarthquakesByLastWeek({ buildResponse, handleHttpError }),
);
...
router.get(
  '/location',
  getEarthquakesLocation({ buildResponse, handleHttpError }),
);

// NOTE: Retrieve earthquake events by geographic region (e.g., Calabria)
router.get(
  '/region',
  getEarthquakesByRegion({ buildResponse, handleHttpError }),
);
...
router.get(
  '/range-time',
  getEarthquakesByDateRange({ buildResponse, handleHttpError }),
);

// NOTE: Filter earthquake events by magnitude (e.g., ≥ 4.0)
router.get(
  '/magnitude',
  getEarthquakesByMagnitude({ buildResponse, handleHttpError }),
);
