  ...
  return async (req, res) => {
    try {
      ...
      if (!Array.isArray(stations)) {
        return handleHttpError(
          res,
          'Invalid station data format received',
          500,
        );
      }
      ...
      if (filteredStation.length === 0) {
        return handleHttpError(
          res,
          `No station found with code '${code}'`,
          404,
        );
      }
  ...
  return async (req, res) => {
    try {
      ...
      if (!stations) {
        return handleHttpError(
          res,
          'Invalid stations data format received',
          500,
        );
      }
  ...
  return async (req, res) => {
    try {
      ...
      if (filteredStationOpen.length === 0) {
        return handleHttpError(
          res,
          'No station found restrictedStatus open',
          404,
        );
      }
  ...
  return async (req, res) => {
    try {
      ...
      const filteredStationClosed = stations.filter(
        (stations) =>
          stations?.$?.restrictedStatus === 'closed' ||
          stations?.$?.restrictedStatus === 'inactive' ||
          stations?.$?.restrictedStatus === 'deprecated' ||
          stations?.$?.restrictedStatus === 'unavailable',
      );
      ...
      const startIndex = (page - 1) * limit;
      const endIndex = startIndex + limit;
      const paginatedStations = filteredStationClosed.slice(
        startIndex,
        endIndex,
      );
      ...
      if (filteredStationClosed.length === 0) {
        return handleHttpError(
          res,
          'No station found restrictedStatus closed',
          404,
        );
      }
