...
export const signUp = ({
  ...
  return async (req, res) => {
    try {
      ...
      if (existingUser) {
        if (existingUser.deleted) {
          ...
          return res
            .status(200)
            .json(
              buildResponse(req, 'registration_successful', user, null, {}),
            );
        }

        // If user exists and is active, return conflict error
        return handleHttpError(res, 'email_exists', 409);
      }
      ...
      return res
        .status(201)
        .json(buildResponse(req, 'registration_successful', user, null, {}));
    } catch (error) {
      ...
      return handleHttpError(res, 'server_error', 500);
    }
  };
};
  ...
  return async (req, res) => {
    try {
      ...
      const user = await User.findOne({ email: req.body.email })
        .select(
          'password name email role experience student bio location website portfolio github linkedin githubId googleId',
        )
        .lean();
      ...
      if (!user.password) {
        return handleHttpError(res, 'already_registered', 400);
      }

      // Compare provided password with hashed one
      const isPasswordValid = await compare(req.body.password, user.password);
      if (!isPasswordValid) {
        return handleHttpError(res, 'incorrect_password', 401);
      }
      ...
      return res.status(200).json(
        buildResponse(req, 'login_successfully', user, null, {
...
export const googleAuthCallback =
  ...
  (req, res) => {
    try {
      ...
      const failureUrl =
        process.env.FRONTEND_DEVELOPMENT || 'http://localhost:5173';
